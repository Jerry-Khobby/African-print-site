...
const FeaturedProducts = () => {
    ...
    //Write a function that will handle the dispatch 
    const handleProductClick = (randomItem) => {
dispatch(selectProduct(randomItem));
    }
...
<div className='grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-2 justify-items-center cursor-pointer'>
{
    randomItems.map((randomItem)=>{
        return(
<div className="border-2 border-gray-200 rounded-lg p-4 w-48 h-48 shadow-sm hover:bg-black hover:text-white" key={randomItem.id} onClick={handleProductClick}>
